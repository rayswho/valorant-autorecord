...
const gameEndURI =
  "/riot-messaging-service/v1/message/ares-match-details/match-details/v1/matches";
...
async function loadPlayerData(
  ...
  for (const puuid of puuids) {
    ...
    const mmrData = await val.requestRemotePD(
    ...
    await fs.writeFile(
      path.join(mmrPath, `${puuid}.json`),
      JSON.stringify(mmrData),
      "utf-8"
    );

    await asyncTimeout(requestDelay);
    const matchHistoryData = await val.requestRemotePD(
      `match-history/v1/history/${puuid}?endIndex=25`,
      shard,
      {
        "X-Riot-ClientPlatform": clientPlatform,
        "X-Riot-ClientVersion": gameVersion,
    ...
    await fs.writeFile(
      path.join(matchHistoryPath, `${puuid}.json`),
...
async function waitForOBS(
  ...
  while (true) {
    try {
      await obs.connect(
        `ws://${connectionSettings.ip}:${connectionSettings.port}`,
      ...
      if (
        ...
        e.code === authenticationConnectionError
      ) {
...
async function main() {
  ...
  const obs = await run(async (): Promise<OBSWebSocket | undefined> => {
    ...
    const connectionSettings = await run(
      async (): Promise<ConnectionSettings> => {
        ...
        try {
          const obsConfig = JSON.parse(
            await fs.readFile(obsConfigPath, "utf-8")
          );
          ...
          throw new Error(
            `Failed to get OBS connection settings from ${obsConfigPath}, error: ${e}`
          );
        }
      }
    );
  ...
  val.on(
    ...
    async (
      ...
      console.log(
        `Loaded ${Object.keys(help.events).length} events, waiting for game...`
      );

      const ws = new WebSocket(
        `wss://riot:${lockfileData.password}@127.0.0.1:${lockfileData.port}`,
        {
      ...
      ws.on("open", () => {
        ...
        Object.entries(help.events).forEach(([name, desc]) => {
          if (name === "OnJsonApiEvent" || alwaysSubscribe.includes(name))
            return;
          ...
          if (data.uri === gameEndURI) {
            if (gameID === null) {
              ...
              if (obs !== undefined) {
                const response = (await obs.call(
                  "StopRecord"
                )) as unknown as StopRecordResponse;
              ...
              const matchData = await val.requestRemotePD<ValorantMatchData>(
                `match-details/v1/matches/${gameID}`,
                initData.shard,
                {
                  "X-Riot-ClientPlatform": clientPlatform,
                  "X-Riot-ClientVersion": gameVersion,
              ...
              if (outputPath !== null) {
                if (mapData === null) {
                  ...
                  ).json()) as MapDataResponse;
                }
                if (agentData === null) {
                  agentData = (await (
                    await fetch("https://valorant-api.com/v1/agents")
                  ).json()) as AgentDataResponse;
                }
                let score = "";
                const ownPlayer = matchData.players.find(
                  (player) => player.subject === chatSession.puuid
                );
                if (ownPlayer !== undefined && matchData.teams !== null) {
                  const ownScore = matchData.teams.find(
                    (team) => team.teamId === ownPlayer.teamId
                  )!.numPoints;

                  if (matchData.teams.length === 1) {
                    score = ownScore.toString();
                  } else {
                    const maxScore = matchData.teams
                      .filter((team) => team.teamId !== ownPlayer.teamId)
                      .sort(
                        (t1, t2) => t2.numPoints - t1.numPoints
                      )[0].numPoints;
                ...
                const newPath = Mustache.render(config.obs.renameTemplate, {
                  ...
                  map:
                    mapData.data.find(
                      (map) => map.mapUrl === matchData.matchInfo.mapId
                    )?.displayName || "",
                  agent:
                    agentData.data.find(
                      (agent) => agent.uuid === ownPlayer?.characterId
                    )?.displayName || "",
              ...
              if (config.data.enable && dataDir !== null) {
                await fs.writeFile(
                  ...
                  JSON.stringify(websocketEvents),
                  "utf-8"
                );
                await fs.writeFile(
                  path.join(dataDir, "match.json"),
            ...
            if (gameID === null) {
              ...
              if (id !== previousGameID) {
                ...
                if (config.data.enable) {
                  ...
                  const gameVersion = loadGameVersion(val, chatSession.puuid);
                  const coreGameData =
                    await val.requestRemoteGLZ<CoregameMatchData>(
                      `core-game/v1/matches/${gameID}`,
                      initData.region,
                      initData.shard,
                      {
                        "X-Riot-ClientPlatform": clientPlatform,
                  ...
                  await fs.writeFile(
                    path.join(dataDir, "coregame-match.json"),
                    JSON.stringify(coreGameData),
                    "utf-8"
                  );
                  await asyncTimeout(requestDelay);
                  const loadoutsData = await val.requestRemoteGLZ(
                    `core-game/v1/matches/${gameID}/loadouts`,
                    initData.region,
                    initData.shard,
                    {
                      "X-Riot-ClientPlatform": clientPlatform,
                  ...
                  await fs.writeFile(
                    path.join(dataDir, "loadouts.json"),
                    JSON.stringify(loadoutsData),
                    "utf-8"
                  );

                  const puuids = coreGameData.Players.map(
            ...
            if (preGameID === null) {
              ...
              if (config.data.enable) {
                ...
                const gameVersion = loadGameVersion(val, chatSession.puuid);
                const pregameMatchData: PregameMatchData =
                  await val.requestRemoteGLZ(
                    `pregame/v1/matches/${preGameID}`,
                    initData.region,
                    initData.shard,
                    {
                      "X-Riot-ClientPlatform": clientPlatform,
                ...
                await fs.writeFile(
                  path.join(dataDir, "pregame-match.json"),
                  JSON.stringify(pregameMatchData),
                  "utf-8"
                );
              }
            }
